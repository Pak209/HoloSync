...
export interface UserHolobot {
  ...
  lastSyncWorkoutDate?: string; // ISO date string of last sync training
  syncWorkoutCountToday?: number; // Number of sync workouts today (should be 0 or 1)
}

export type PlayerRank = 'Common' | 'Champion' | 'Rare' | 'Elite' | 'Legendary';

export interface UserProfile {
  id: string;
  username: string;
  email?: string; // Add email property for auth
  ...
  playerRank?: PlayerRank; // Add player rank for workout rewards
  dailyWorkoutCount?: number; // Track daily workouts for rank limits
  lastWorkoutReset?: string; // ISO date string for daily workout reset
  arena_passes?: number;
  exp_boosters?: number;
  ...
  sync_points?: number; // Add sync points for fitness tracking
}
...
export function mapDatabaseToUserProfile(dbProfile: any): UserProfile {
  if (dbProfile.username) {
    ...
    return {
      ...
      playerRank: dbProfile.player_rank || 'Common',
      dailyWorkoutCount: dbProfile.daily_workout_count || 0,
      lastWorkoutReset: dbProfile.last_workout_reset || new Date().toISOString(),
      ...
      sync_points: dbProfile.sync_points || 0 // Initialize sync points
    };
  } else {
