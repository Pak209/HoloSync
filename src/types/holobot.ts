...
export function getHolobotByName(name: string): HolobotStats | undefined {
    const key = Object.keys(HOLOBOT_STATS).find(
        k => HOLOBOT_STATS[k].name.toLowerCase() === name.toLowerCase()
    );
    return key ? HOLOBOT_STATS[key] : undefined;
} 