...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // const { clearUserData } = useRewardStore(); // TODO: Import rewardStore when available
      ...
      try {
        ...
        const { data: authListener } = supabase.auth.onAuthStateChange(async (event, session) => {
          ...
          if (event === 'SIGNED_OUT') {
            setCurrentUser(null);
            setError(null);
            // clearUserData(); // TODO: Clear reward system data when signing out
          }
          else if (session) {
  ...
  const login = async (email: string, password: string) => {
    ...
    try {
      ...
      showToast("Login Failed", err instanceof Error ? err.message : "Failed to log in", "destructive");
      throw err;
    } finally {
      setLoading(false);
    }
  ...
  const logout = async () => {
    ...
    try {
      ...
      setCurrentUser(null);
      // clearUserData(); // TODO: Clear reward system data when logging out
      
      showToast("Logged Out", "You have been logged out");
    } catch (err) {
      logDebug('Logout error:', err);
      setError(err instanceof Error ? err.message : "Logout failed");
      showToast("Logout Failed", err instanceof Error ? err.message : "Failed to log out", "destructive");
    } finally {
      setLoading(false);
    }
  };
  ...
  const signup = async (email: string, password: string, username: string) => {
    ...
    try {
      ...
      showToast("Signup Successful", "Your account has been created");
    } catch (err) {
      logDebug('Signup error:', err);
      setError(err instanceof Error ? err.message : "Signup failed");
      showToast("Signup Failed", err instanceof Error ? err.message : "Failed to create account", "destructive");
      throw err;
    } finally {
      setLoading(false);
    }
  ...
  const updateUser = async (updates: Partial<UserProfile>): Promise<void> => {
    if (!currentUser) {
      showToast("Error", "You must be logged in to update your profile", "destructive");
      return;
    }
    ...
    try {
      ...
      showToast("Update Failed", error instanceof Error ? error.message : "Failed to update profile", "destructive");
    }
  };
  ...
  const searchPlayers = async (query: string): Promise<UserProfile[]> => {
    try {
      ...
      showToast("Search Failed", error instanceof Error ? error.message : "Failed to search players", "destructive");
      return [];
    }
  };
