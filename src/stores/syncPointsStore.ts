  ...
  persist(
    (set, get) => ({
      ...
      unlockSpecialAttack: (attackId: string) => {
        const state = get();
        const attackIndex = state.specialAttacks.findIndex(a => a.id === attackId);
        
        if (attackIndex < 0) return false;
        
        const attack = state.specialAttacks[attackIndex];
        if (attack.unlocked || !get().canAffordUpgrade(attack.syncPointCost)) {
          return false;
        }
        
        const updatedAttacks = [...state.specialAttacks];
        updatedAttacks[attackIndex] = { ...attack, unlocked: true };
      ...
      unlockAbilityChip: (chipId: string, tierLevel: number) => {
        const state = get();
        const chipIndex = state.abilityChips.findIndex(c => c.id === chipId);
        
        if (chipIndex < 0) return false;
        
        const chip = state.abilityChips[chipIndex];
        if (chip.unlocked || !get().canAffordUpgrade(chip.syncPointCost)) {
          return false;
        }
        
        const updatedChips = [...state.abilityChips];
        updatedChips[chipIndex] = { ...chip, unlocked: true };
